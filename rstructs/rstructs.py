...
class LuaScripts(object):
    ...
    LREMINDEX_COPY = """
        ...
        if (argv1_num ~= 0 and argv1_num ~= -1) then
            ...
            redis.call('rpush', TEMP_KEY, unpack(r));
        elseif argv1_num == 0 then
            local r = redis.call('lrange', KEYS[1], 1, -1);
            redis.call('rpush', TEMP_KEY, unpack(r));
        elseif argv1_num == -1 then
            local r = redis.call('lrange', KEYS[1], 0, -2);
            redis.call('rpush', TEMP_KEY, unpack(r));
        end
